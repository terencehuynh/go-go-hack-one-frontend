      ...
      .then(successResponse => {
        ...
        for (var i = 0; i < searchResponses.length; i++) {
          ...
          if (!(searchResponses[i].name in responseData[entityId])) {
            responseData[entityId][searchResponses[i].name] =
              searchResponses[i].attribute_value;
          }
        }
        ...
        if (
          queryParams.queryName !== undefined &&
        ...
        this.setState({
          ...
          loading: false,
          queryStringShow: queryStringShow
        });
      })
      .catch(error => {
  ...
  render() {
    return (
      <section className="section" style={{}}>
        <div style={{ textAlign: "center" }} onClick={this.replaceMeLater}>
          ...
          this.state.loading === false ? (
            <ListGroup>
              {Object.keys(this.state.searchResponseData).map((k, i) => (
                <ListItem
                  key={i}
                  dataObject={this.state.searchResponseData[k]}
                />
              ))}
            </ListGroup>
          ) : null}
